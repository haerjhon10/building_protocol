...
st.title("מחולל פרוטוקול מסירת מבנה")

if 'sections' not in st.session_state:
    st.session_state.sections = {}

title = st.text_input("כותרת המסמך", "שם הפרויקט")

# Space management with aligned input and button
col1, col2, col3 = st.columns([0.6, 0.2, 0.2])
with col1:
    new_section_name = st.text_input("הוסף חלל חדש")
with col2:
    st.write("")  # Add vertical spacing to align with text input
    if st.button(" הוסף חלל", use_container_width=True) and new_section_name:
...
for section_name in list(st.session_state.sections.keys()):
    ...
    if st.button(f" מחיקת חלל {section_name}"):
        del st.session_state.sections[section_name]
        st.rerun()
        
    # Test management
    test_input = st.text_input(f"הוסף בדיקה נדרשת ל {section_name}", key=f"test_{section_name}")
    if st.button(f"הוסף בדיקה ל {section_name}", key=f"add_test_{section_name}"):
        if test_input:
...
if st.button("צור מסמך"):
    ...
    with open(file_path, "rb") as f:
        ...
        href = f'<a href="data:application/octet-stream;base64,{b64}" download="inspection_protocol.docx">לחץ להורדת המסמך</a>'
